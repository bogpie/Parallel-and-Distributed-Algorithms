...
// macro definition for minimum
#define MIN(x, y) (((x) < (y)) ? (x) : (y))

// read the input, taking into account the number of threads
...
// returns a negative value if a is prioritised, and a positive value otherwise 
...
void free_generation(individual *generation)
{
    ...
    if (generation != NULL)
        for (i = 0; i < generation->chromosome_length; ++i)
        {
            ...
            if (generation[i].chromosomes != NULL)
            {
...
void mergeSort(struct my_arg *data)
{
    ...
    for (width = 1; width < data->object_count; width = 2 * width)
    {
        ...
        int end = MIN((data->id + 1) * merges / data->noThreads * 2 * width, data->object_count);
        ...
        while (i < end)
        {
            ...
            merge(data->current_generation, portionStart, portionMid, portionEnd, data->sorted);
        ...
        // swap the new sorted array with the current generation one
        data->tmp = data->current_generation;
        data->current_generation = data->sorted;
        data->sorted = data->tmp;
...
void *thread_function(void *arg)
{
    ...
    for (int k = 0; k < generations_count; ++k)
    {
        ...
        // sort the current generation after fitness has been calculated for all individuals
        mergeSort(data);
        pthread_barrier_wait(data->barrier);
                               ...
                               int noThreads)
{
    ...
    individual *sorted = (individual *)calloc(object_count, sizeof(individual));
    ...
    for (int idThread = 0; idThread < noThreads; ++idThread)
    {
        ...
        arguments[idThread].sorted = sorted;
        arguments[idThread].barrier = barrier;
